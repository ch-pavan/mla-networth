...
export default function Home() {
  ...
  return <><a className="skipLink" href="#main-content">Skip to the database</a><main id="main-content">
    ...
    <section id="top" className="hero">
      ...
      <div className="ticker"><span>IN NUMBERS</span><div><b>{allData.length?fmt(totalDeclaredNetWorth):((chamber==="all"?allError:dataErrors[chamberErrorKey])?"Unavailable":"Loading")}</b><small>total declared net worth · {allData.length?`${allData.length.toLocaleString("en-IN")} ${chamberLabel}`:chamberLabel}</small></div>{chamber==="all"?<div className="tickerHouses"><div className="tickerHouseRow"><span><b>{mlaCount.toLocaleString("en-IN")}</b> MLA</span><span><b>{lsCount.toLocaleString("en-IN")}</b> LS</span><span><b>{rsCount.toLocaleString("en-IN")}</b> RS</span></div><small>sitting records by house</small></div>:<div><b>{chamber==="lok_sabha"?lsCount.toLocaleString("en-IN"):chamber==="rajya_sabha"?rsCount.toLocaleString("en-IN"):mlaCount.toLocaleString("en-IN")}</b><small>{chamber==="lok_sabha"?"Lok Sabha seats":chamber==="rajya_sabha"?"Rajya Sabha members":"sitting MLAs"}</small></div>}<div><b>{candidateIndex?.meta.candidateRecords.toLocaleString("en-IN")??(dataErrors.candidateIndex?"Unavailable":"Loading")}</b><small>candidate affidavits</small></div>{chamber!=="rajya_sabha"&&<div><b>{winnerTicker}</b><small>{winnerTickerLabel}</small></div>}<div><b>{historyTicker}</b><small>{historyTickerLabel}</small></div></div>
    </section>
