...
type MLA = {
  ...
  electionFolder?: string; candidateId?: number;
};
...
type Chamber = "assembly" | "lok_sabha" | "rajya_sabha";
...
export default function Home() {
  ...
  const [chamber,setChamber]=useState<Chamber>("assembly");
  ...
  const activeSnapshot=chamber==="lok_sabha"?mpSnapshot:chamber==="rajya_sabha"?rsSnapshot:snapshot;
  ...
  const allData=useMemo(()=>{
    ...
    return activeSnapshot.records.flatMap((r):MLA[]=>{
      ...
      const timeline=chamber==="assembly"
        ?buildVerifiedAssetHistory({state:r.state,electionYear,name:r.name,assets:assetsRupees,sourceUrl:activeSnapshot.meta.sourceUrl},segments)
        :[{year:electionYear||new Date().getFullYear(),assets:assetsRupees,sourceUrl:r.candidateUrl||activeSnapshot.meta.sourceUrl}];
  ...
  const currentWatchItem=(representative:MLA):WatchItem=>({
    id:`${chamber}:${representative.sourceRank??normalizeName(`${representative.state}-${representative.name}-${representative.constituency}`)}`,
    label:`${representative.name} — ${representative.constituency}, ${representative.state}`,
    href:chamber==="lok_sabha" && representative.electionFolder && representative.candidateId
      ?publicUrl(`/person?type=candidate&election=${encodeURIComponent(representative.electionFolder)}&id=${representative.candidateId}`)
      :chamber==="rajya_sabha"
        ?(representative.sourceUrl||activeSnapshot?.meta.sourceUrl||"https://adrindia.org/")
        :publicUrl(`/person?type=current&rank=${representative.sourceRank??1}`),
  });
  const chamberLabel=chamber==="lok_sabha"?"Lok Sabha MPs":chamber==="rajya_sabha"?"Rajya Sabha MPs":"sitting MLAs";
  ...
  return <><a className="skipLink" href="#main-content">Skip to the database</a><main id="main-content">
    <header className="topbar">
      ...
      <nav><a href="#explore">{chamber==="assembly"?"Current MLAs":"Current MPs"}</a><a href="#candidates">Candidate DB</a>{chamber!=="rajya_sabha"&&<a href="#archive">Seats</a>}<Link href="/map">Map</Link><a href="#analysis">Analysis</a><a href="#method">Method</a></nav>
    ...
    <section id="top" className="hero">
      ...
      <div className="heroGrid">
        <div><h1>Follow the money.<br/><em>Know your neta.</em></h1><p className="dek">India&apos;s most ambitious public record of the wealth declared by elected representatives—across elections, parties and 4,123 assembly constituencies.</p>
          <div className="searchBox"><span aria-hidden="true">⌕</span><input id="search" aria-label="Search representatives" value={query} onChange={e=>{setQuery(e.target.value);setMlaLimit(12)}} placeholder={chamber==="assembly"?"Search an MLA, constituency, party or state…":"Search an MP, state, party…"}/>{query&&<button className="clearSearch" type="button" onClick={()=>setQuery("")} aria-label="Clear representative search">×</button>}<kbd>⌘/Ctrl K</kbd></div>
          <div className="quick"><span>TRY</span>{(chamber==="lok_sabha"?["Narendra Modi","Guntur","BJP"]:chamber==="rajya_sabha"?["Jaya Bachchan","Telangana","BJP"]:["D K Shivakumar","Karnataka","BJP"]).map(x=><button key={x} onClick={()=>{setQuery(x);setMlaLimit(12);document.getElementById("explore")?.scrollIntoView({behavior:"smooth"})}}>{x}</button>)}</div>
        </div>
        {headline?<aside className="headlineCard"><div className="cardKicker">BIGGEST DECLARED FORTUNE</div><div className="rank">01</div><h2>{headline.name}</h2><p>{headline.constituency} · {headline.state}</p><div className="bigMoney">{fmt(headline.assets)}</div><div className="rise">{headline.historical!==false?`↗ ${headline.growth}%`:(chamber==="lok_sabha"?"2024 Lok Sabha winners":chamber==="rajya_sabha"?"ADR March 2026":"2025 national snapshot")} <span>{headline.historical!==false?"since previous affidavit":"ADR rank #1"}</span></div><Sparkline values={headline.values}/><button onClick={()=>window.location.assign(profileHref(headline))}>View the full record →</button></aside>:<aside className="headlineCard" role={dataErrors[chamberErrorKey]?"alert":"status"}><div className="cardKicker">CURRENT SNAPSHOT</div><h2>{dataErrors[chamberErrorKey]?"Data unavailable":"Loading public records…"}</h2><p>{dataErrors[chamberErrorKey]??(chamber==="lok_sabha"?"Opening the Lok Sabha winner index.":chamber==="rajya_sabha"?"Opening the Rajya Sabha sitting-MP index.":"Opening the nationwide sitting-MLA index.")}</p>{dataErrors[chamberErrorKey]&&<button onClick={()=>retryData(chamberErrorKey)}>Retry data →</button>}</aside>}
      </div>
      <div className="ticker"><span>IN NUMBERS</span><div><b>{allData.length?fmt(totalDeclaredNetWorth):(dataErrors[chamberErrorKey]?"Unavailable":"Loading")}</b><small>total declared net worth · {allData.length?`${allData.length.toLocaleString("en-IN")} ${chamberLabel}`:chamberLabel}</small></div><div><b>{chamber==="lok_sabha"?"543":chamber==="rajya_sabha"?"233":"4,123"}</b><small>{chamber==="lok_sabha"?"Lok Sabha seats":chamber==="rajya_sabha"?"Rajya Sabha seats":"assembly constituencies"}</small></div><div><b>{candidateIndex?.meta.candidateRecords.toLocaleString("en-IN")??(dataErrors.candidateIndex?"Unavailable":"Loading")}</b><small>candidate affidavits</small></div><div><b>{chamber==="rajya_sabha"?"—":((activeArchive?.meta.winnerRecords??(chamber==="lok_sabha"?undefined:archive?.meta.winnerRecords))?.toLocaleString("en-IN")??(dataErrors.archive?"Unavailable":"On demand"))}</b><small>{chamber==="rajya_sabha"?"no seat archive yet":"historical winners"}</small></div><div><b>{chamber==="assembly"?(history?.meta.comparisonCount.toLocaleString("en-IN")??(dataErrors.history?"Unavailable":"On demand")):"—"}</b><small>{chamber==="assembly"?"asset comparisons":"history soon"}</small></div></div>
    </section>

    <section id="explore" className="section explorer" ref={explorerSection}>
      <div className="sectionHead"><div><span className="sectionNo">01 / EXPLORE</span><h2>The wealth table</h2><p>{chamber==="lok_sabha"?"Search sitting Lok Sabha MPs from the latest general-election winners.":chamber==="rajya_sabha"?"Search sitting Rajya Sabha MPs from ADR’s March 2026 national analysis.":"Search the nationwide 2025 sitting-MLA affidavit snapshot."}</p></div><div className="dataStamp"><i></i> {chamber==="lok_sabha"?`LOK SABHA WINNERS · ${activeSnapshot?`${activeSnapshot.meta.recordCount.toLocaleString("en-IN")} RECORDS`:(dataErrors.mpSnapshot?"DATA UNAVAILABLE":"LOADING")}`:chamber==="rajya_sabha"?`ADR RAJYA SABHA · ${activeSnapshot?`${activeSnapshot.meta.recordCount.toLocaleString("en-IN")} RECORDS`:(dataErrors.rsSnapshot?"DATA UNAVAILABLE":"LOADING")}`:`ADR NATIONAL REPORT · ${snapshot?`${snapshot.meta.recordCount.toLocaleString("en-IN")} RECORDS`:(dataErrors.snapshot?"DATA UNAVAILABLE":"LOADING")}`}</div></div>
    ...
    {chamber!=="rajya_sabha"&&<section id="archive" className="archive section" ref={archiveSection}>
