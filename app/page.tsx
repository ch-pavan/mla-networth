...
export default function Home() {
  ...
  useEffect(()=>{Promise.all([fetch("/data/adr-sitting-mlas-2025.json").then(r=>r.json()),fetch("/data/adr-recontest-history.json").then(r=>r.json())]).then(([current,historical]:[AdrSnapshot,HistorySnapshot])=>{setSnapshot(current);setHistory(historical)}).catch(()=>{})},[]);
  ...
  return <main>
    ...
    <section id="top" className="hero">
      ...
      <div className="ticker"><span>IN NUMBERS</span><div><b>4,123</b><small>assembly constituencies</small></div><div><b>{snapshot?.meta.recordCount.toLocaleString("en-IN")??"Loading"}</b><small>sitting MLAs indexed</small></div><div><b>{history?.meta.comparisonCount.toLocaleString("en-IN")??"—"}</b><small>historical comparisons</small></div><div><b>{history?.meta.snapshotMatchCount.toLocaleString("en-IN")??"—"}</b><small>strictly matched timelines</small></div><div><b>{history?`${history.meta.firstYear}—${history.meta.latestYear}`:"—"}</b><small>election years covered</small></div></div>
    </section>
    ...
    <section id="method" className="method"><div><span className="sectionNo">SOURCE NOTE</span><h2>Public records,<br/>carefully connected.</h2></div><div><p>Every figure shown as a declaration originates in a candidate&apos;s self-sworn election affidavit. “Net worth” here means declared total assets minus declared liabilities; it is not independently audited market wealth.</p><div className="methodLinks"><a href="https://affidavit.eci.gov.in/" target="_blank">Election Commission affidavit portal ↗</a><a href="https://adrindia.org/sites/default/files/All_India_Sitting_MLAs_Report_2025_English.pdf" target="_blank">ADR national MLA report ↗</a><a href="https://www.myneta.info/" target="_blank">MyNeta comparison records ↗</a></div><small>The nationwide snapshot contains 4,092 sitting MLAs extracted from ADR&apos;s report published 17 March 2025, covering 28 state assemblies and three Union Territories. Historical coverage adds 7,723 MyNeta asset comparisons from 94 election datasets spanning 2004—2025. A conservative year, state, normalized-name and exact-asset check links 1,376 current profiles; older points are added only when adjacent affidavit values form an exact chain. Individual values should be checked against the linked primary affidavit before investigative or legal use.</small></div></section>
