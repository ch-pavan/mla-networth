...
export default function Home() {
  ...
  useEffect(()=>{Promise.all([fetch("/data/adr-sitting-mlas-2025.json").then(r=>r.json()),fetch("/data/adr-recontest-history.json").then(r=>r.json()),fetch("/data/adr-winner-archive.json").then(r=>r.json())]).then(([current,historical,winners]:[AdrSnapshot,HistorySnapshot,WinnerArchive])=>{setSnapshot(current);setHistory(historical);setArchive(winners)}).catch(()=>{})},[]);
  ...
  return <main>
    <header className="topbar">
      ...
      <nav><a href="#explore">Explore</a><a href="#archive">Archive</a><a href="#states">States</a><a href="#analysis">Analysis</a><a href="#method">Methodology</a></nav>
    ...
    <section id="top" className="hero">
      ...
      <div className="ticker"><span>IN NUMBERS</span><div><b>4,123</b><small>assembly constituencies</small></div><div><b>{snapshot?.meta.recordCount.toLocaleString("en-IN")??"Loading"}</b><small>sitting MLAs indexed</small></div><div><b>{archive?.meta.winnerRecords.toLocaleString("en-IN")??"—"}</b><small>historical winner records</small></div><div><b>{history?.meta.comparisonCount.toLocaleString("en-IN")??"—"}</b><small>asset comparisons</small></div><div><b>{history?`${history.meta.firstYear}—${history.meta.latestYear}`:"—"}</b><small>election years covered</small></div></div>
    </section>
    ...
    <section id="method" className="method"><div><span className="sectionNo">SOURCE NOTE</span><h2>Public records,<br/>carefully connected.</h2></div><div><p>Every figure shown as a declaration originates in a candidate&apos;s self-sworn election affidavit. “Net worth” here means declared total assets minus declared liabilities; it is not independently audited market wealth.</p><div className="methodLinks"><a href="https://affidavit.eci.gov.in/" target="_blank">Election Commission affidavit portal ↗</a><a href="https://adrindia.org/sites/default/files/All_India_Sitting_MLAs_Report_2025_English.pdf" target="_blank">ADR national MLA report ↗</a><a href="https://www.myneta.info/" target="_blank">MyNeta election archive ↗</a></div><small>The nationwide snapshot contains 4,092 sitting MLAs extracted from ADR&apos;s report published 17 March 2025. The constituency archive adds 13,916 winner records from 121 state-election folders, while historical growth uses 7,723 MyNeta asset comparisons from 94 comparison datasets spanning 2004—2025. A conservative year, state, normalized-name and exact-asset check links 1,376 current profiles; older points are added only when adjacent affidavit values form an exact chain. Constituency labels remain exactly as published and may reflect delimitation or spelling changes. Individual values should be checked against the linked primary affidavit before investigative or legal use.</small></div></section>
