...
export default function Home() {
  ...
  return <><a className="skipLink" href="#main-content">Skip to the database</a><main id="main-content">
    ...
    <section id="top" className="hero">
      ...
      <div className="ticker"><span>IN NUMBERS</span><div><b>{allData.length?fmt(totalDeclaredNetWorth):(dataErrors.snapshot?"Unavailable":"Loading")}</b><small>total declared net worth · {allData.length?`${allData.length.toLocaleString("en-IN")} sitting MLAs`:"ADR 2025 snapshot"}</small></div><div><b>4,123</b><small>assembly constituencies</small></div><div><b>{candidateIndex?.meta.candidateRecords.toLocaleString("en-IN")??(dataErrors.candidateIndex?"Unavailable":"Loading")}</b><small>candidate affidavits</small></div><div><b>{archive?.meta.winnerRecords.toLocaleString("en-IN")??(dataErrors.archive?"Unavailable":"On demand")}</b><small>historical winners</small></div><div><b>{history?.meta.comparisonCount.toLocaleString("en-IN")??(dataErrors.history?"Unavailable":"On demand")}</b><small>asset comparisons</small></div></div>
    </section>
