...
type MLA = {
  ...
  historical?: boolean; education?: string; age?: number | null; gender?: string; sourceRank?: number;
};
...
export default function Home() {
  ...
  useEffect(()=>{fetch("/data/adr-sitting-mlas-2025.json").then(r=>r.json()).then((data:AdrSnapshot)=>setSnapshot(data)).catch(()=>{})},[]);
  const allData=useMemo(()=>snapshot?snapshot.records.map((r):MLA=>({name:r.name,state:r.state,constituency:r.constituency,party:r.party,assets:r.assets/1e7,liabilities:r.liabilities/1e7,growth:0,years:[r.electionYear,r.electionYear],values:[r.assets/1e7,r.assets/1e7],cases:r.criminalCases,historical:false,education:r.education,age:r.age,gender:r.gender,sourceRank:r.rank})):mlaData,[snapshot]);
  const states=["All India",...Array.from(new Set(allData.map(m=>m.state))).sort()];
  const filtered=useMemo(()=>allData.filter(m=>(state==="All India"||m.state===state)&&`${m.name} ${m.constituency} ${m.party}`.toLowerCase().includes(query.toLowerCase())).sort((a,b)=>b[sort]-a[sort]),[allData,query,state,sort]);
  ...
  const totalAssets=allData.reduce((a,b)=>a+b.assets,0);
  ...
  return <main>
    ...
    <section id="top" className="hero">
      ...
      <div className="ticker"><span>IN NUMBERS</span><div><b>4,123</b><small>assembly constituencies</small></div><div><b>{snapshot?.meta.recordCount.toLocaleString("en-IN")??"Loading"}</b><small>ADR sitting MLAs indexed</small></div><div><b>₹{(totalAssets/100000).toFixed(1)}L Cr</b><small>total declared assets indexed</small></div><div><b>{states.length-1}</b><small>states &amp; UTs represented</small></div><div><b>2025</b><small>national snapshot</small></div></div>
    </section>
