...
export default function Home() {
  ...
  const [candidateHistory,setCandidateHistory]=useState<(CandidateRecord&{electionYear:number})[]>([]);
  ...
  useEffect(()=>{const election=candidateElections.find(e=>e.electionYear===candidateYear);if(!election)return;setCandidateShard(null);fetch(election.file).then(r=>r.json()).then((data:CandidateShard)=>{setCandidateShard(data);setSelectedCandidate(data.records.reduce((a,b)=>b.assets>a.assets?b:a,data.records[0]))}).catch(()=>{})},[candidateElections,candidateYear]);
  useEffect(()=>{if(!selectedCandidate||!candidateIndex)return;const elections=candidateIndex.states.find(s=>s.state===candidateState)?.elections??[];Promise.all(elections.map(e=>fetch(e.file).then(r=>r.json()).then((d:CandidateShard)=>d.records.filter(row=>row.normalizedName===selectedCandidate.normalizedName).map(row=>({...row,electionYear:e.electionYear}))))).then(groups=>setCandidateHistory(groups.flat().sort((a,b)=>a.electionYear-b.electionYear))).catch(()=>setCandidateHistory([]))},[selectedCandidate,candidateIndex,candidateState]);
  ...
  return <main>
    ...
    <section id="candidates" className="candidateDb section">
      ...
      <div className="candidateControls"><label><span>STATE / UT</span><select value={candidateState} onChange={e=>{const next=e.target.value;setCandidateState(next);setCandidateYear(candidateIndex?.states.find(s=>s.state===next)?.elections[0]?.electionYear??2023);setSelectedCandidate(null)}}>{candidateIndex?.states.map(s=><option key={s.state}>{s.state}</option>)}</select></label><label><span>ELECTION</span><select value={candidateYear} onChange={e=>{setCandidateYear(Number(e.target.value));setSelectedCandidate(null)}}>{candidateElections.map(e=><option key={e.electionFolder} value={e.electionYear}>{e.electionYear} · {e.candidateCount.toLocaleString("en-IN")} candidates</option>)}</select></label><label className="candidateSearch"><span>SEARCH THIS ELECTION</span><input value={candidateQuery} onChange={e=>setCandidateQuery(e.target.value)} placeholder="Candidate, constituency or party…"/></label></div>
      <div className="candidateWorkspace"><div className="candidateList"><div className="candidateListHead"><span>{candidateShard?`${candidateShard.meta.candidateCount.toLocaleString("en-IN")} candidates in ${candidateState} ${candidateYear}`:"Loading election…"}</span><small>Top 50 matches by declared assets</small></div>{candidateResults.map(row=><button key={row.candidateId} className={selectedCandidate?.candidateId===row.candidateId?"active":""} onClick={()=>setSelectedCandidate(row)}><div><strong>{row.name}</strong><small>{row.constituency} · {row.party}</small></div><span>{fmt(row.assets/1e7)}</span></button>)}</div>
        <aside className="candidateProfile">{selectedCandidate?<><div className="candidateIdentity"><span>INTERNAL CANDIDATE PROFILE</span><h3>{selectedCandidate.name}</h3><p>{selectedCandidate.party} · {selectedCandidate.constituency} · {candidateState} {candidateYear}</p></div><div className="candidateMoney"><div><small>DECLARED ASSETS</small><b>{fmt(selectedCandidate.assets/1e7)}</b></div><div><small>LIABILITIES</small><b>{fmt(selectedCandidate.liabilities/1e7)}</b></div><div><small>ASSETS LESS LIABILITIES</small><b>{fmt((selectedCandidate.assets-selectedCandidate.liabilities)/1e7)}</b></div></div><dl className="candidateFacts"><div><dt>Education</dt><dd>{selectedCandidate.education||"Not declared"}</dd></div><div><dt>Criminal cases</dt><dd>{selectedCandidate.criminalCases}</dd></div><div><dt>Election record ID</dt><dd>{candidateShard?.meta.electionFolder}/{selectedCandidate.candidateId}</dd></div></dl><div className="internalHistory"><div><h4>Connected election history</h4><span>Exact normalized-name match within {candidateState}</span></div>{candidateHistory.map(row=><div key={`${row.electionYear}-${row.candidateId}`}><time>{row.electionYear}</time><i></i><div><strong>{row.constituency}</strong><small>{row.party} · {fmt(row.assets/1e7)} assets · {fmt(row.liabilities/1e7)} liabilities</small></div></div>)}</div><a className="sourceCitation" href={selectedCandidate.candidateUrl} target="_blank">Verify against source record ↗</a></>:<div className="candidateBlank">Select a candidate to open their internal profile.</div>}</aside></div>
    </section>
