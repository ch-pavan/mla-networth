...
export default function Home() {
  ...
  const stateStats=useMemo(()=>Array.from(new Set(allData.map(m=>m.state))).map(s=>{const d=allData.filter(m=>m.state===s);const total=d.reduce((a,b)=>a+b.assets,0);return {state:s, total, avg:d.length?total/d.length:0,count:d.length}}).sort((a,b)=>b.total-a.total).slice(0,8),[allData]);
  ...
  const partyStats=useMemo(()=>Object.entries(parties).map(([party,color])=>{const records=allData.filter(record=>record.party===party);const total=records.reduce((sum,record)=>sum+record.assets,0);return {party,color,count:records.length,total,avg:records.length?total/records.length:0}}).filter(item=>item.count>0).sort((a,b)=>b.total-a.total),[allData]);
  const largestPartyTotal=Math.max(1,...partyStats.map(item=>item.total));
