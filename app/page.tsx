...
import { formatCrores as fmt } from "../lib/format-money";
...
export default function Home() {
  ...
  const seatGroups=useMemo(()=>{
    ...
    return [...groups.values()].map(rows=>rows.sort((a,b)=>a.electionYear-b.electionYear)).sort((a,b)=>(b.at(-1)?.electionYear??0)-(a.at(-1)?.electionYear??0)||a[0].constituency.localeCompare(b[0].constituency));
  ...
  const candidateResults=useMemo(()=>[...(candidateShard?.records??[])].filter(row=>normalizeName(`${row.name} ${row.constituency} ${row.party}`).includes(normalizeName(candidateQuery))).sort((a,b)=>b.assets-a.assets),[candidateShard,candidateQuery]);
