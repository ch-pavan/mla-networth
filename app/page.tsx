...
type WinnerRecord = { state:string;electionYear:number;electionDate?:string;electionFolder:string;rankByAssets:number;candidateId:number;name:string;normalizedName:string;constituency:string;normalizedConstituency:string;party:string;criminalCases:number;education:string;assets:number;liabilities:number;candidateUrl:string };
...
export default function Home() {
  ...
  return <><a className="skipLink" href="#main-content">Skip to the database</a><main id="main-content">
    ...
    <section id="archive" className="archive section" ref={archiveSection}>
      ...
      <div className="seatGrid">{seatGroups.slice(0,seatLimit).map(rows=><article className="seatCard" key={`${rows[0].state}-${rows[0].normalizedConstituency}`}><header><div><span>{rows[0].state}</span><h3>{rows.at(-1)?.constituency}</h3></div><b>{rows.length} elections</b></header><div className="seatTimeline">{rows.map((row,i)=><a href={`/person?type=candidate&election=${encodeURIComponent(row.electionFolder)}&id=${row.candidateId}`} key={`${row.electionFolder}-${row.candidateId}`}><i className={i===rows.length-1?"current":""}></i><time dateTime={row.electionDate}>{row.electionYear}</time><div><strong>{row.name}</strong><small>{row.party} · {fmt(row.assets/1e7)} assets</small></div><span>→</span></a>)}</div></article>)}</div>
