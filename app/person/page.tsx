...
type Current = { rank:number;state:string;electionYear:number;constituency:string;name:string;party:string;age:number|null;gender:string|null;assets:number|null;liabilities:number|null;criminalCases:number|null;seriousCriminalCases:number|null;education:string;panDeclared:boolean|null };
type Profile = {
  kind:"candidate"|"current"; name:string;state:string;year:number;constituency:string;party:string;
  assets:number|null;liabilities:number|null;criminalCases:number|null;education:string;sourceUrl:string;
  electionDate?:string|null;electionType?:string;
  age?:number|null;gender?:string|null;seriousCases?:number|null;panDeclared?:boolean|null;recordId:string;
};
  ...
  return <><a className="skipLink" href="#main-content">Skip to public record</a><main className="personPage" id="main-content">
    ...
    {profile&&<>
      <section className="personHero">
        ...
        <div className="personHeroGrid"><div className="personMonogram">{initials(profile.name)}</div><div><span className="recordType">{profile.kind==="current"?(profile.recordId.startsWith("LS")?"LOK SABHA 2024 WINNER PROFILE":profile.recordId.startsWith("RS")?"SITTING RAJYA SABHA PROFILE":"SITTING MLA PROFILE"):"CANDIDATE AFFIDAVIT PROFILE"}</span><h1>{profile.name}</h1><p><b>{profile.party}</b> · {profile.constituency}, {profile.state}</p></div><div className="personNet"><small>ASSETS LESS LIABILITIES</small><b>{money(netWorth)}</b><span>{netWorth===null?"Requires both declared values":"Based on declared values"}</span></div></div>
      </section>
